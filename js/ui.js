...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Auf GitHub Pages kommt der Export von der Cloudron-App (CORS,
        // FEDIPOL_CORS_ORIGINS), sonst vom selben Origin.
        const CLOUDRON_DATA_URL = 'https://fedipol.wolkenbar.de/fedipol_data.json';
        const dataUrl = window.location.hostname.endsWith('.github.io')
            ? CLOUDRON_DATA_URL
            : 'fedipol_data.json';
        const response = await fetch(dataUrl + '?' + new Date().getTime());
        if (!response.ok) {
